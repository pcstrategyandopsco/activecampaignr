...
This means incremental sync fetches 2 records instead of 1,001, saving
~10 seconds and ~19 API calls.
...
| Switching between AC accounts | [`ac_cache_clear()`](https://pcstrategyandopsco.github.io/activecampaignr/reference/ac_cache_clear.md) then re-auth |
| Cache seems corrupt or stale | [`ac_cache_clear()`](https://pcstrategyandopsco.github.io/activecampaignr/reference/ac_cache_clear.md) |
...
ActiveCampaign’s `updated_after` filter returns modified records, but it
does not tell you about deleted records. If a deal is deleted in AC,
...
`updated_after` filtering on custom field values, so incremental sync is
...
The cache is local to the machine and user. If you run the same pipeline
on CI or a different laptop, it starts with a cold cache. Consider
committing a seed cache file to your project repo if cold start time
matters.
