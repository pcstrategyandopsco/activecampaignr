...
1. **Cache hit** — If data was fetched within the TTL, return it instantly
2. **Incremental sync** — If stored data exists, only fetch recent changes
   and merge them
...
The merge uses the record ID as the key: old versions are removed,
new versions are appended, then deduplicated.
